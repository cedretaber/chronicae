...
function buildWorld() {
  ...
  return { state: s, sellerPolityId, buyerPolityId, provinceId, holdingId }
}
...
describe('applyLandContractTransferGoal', () => {
  it('purchase: transfers grantee and fires LAND_CONTRACT_TRANSFERRED + LAND_CONTRACT_PURCHASED', () => {
    const { state, sellerPolityId, buyerPolityId, holdingId } = buildWorld()
    ...
    expect(after.holdingTerminalPolityCache[holdingId]).toBe(buyerPolityId)
  ...
  it('no-op when grantee already matches', () => {
    const { state, sellerPolityId, holdingId } = buildWorld()
    const ctx = makeCtx(state)
    const result = applyLandContractTransferGoal(ctx, {
      holdingId,
      fromPolityId: sellerPolityId,
  ...
  it('returns error when rank invariant would be violated', () => {
    ...
    const ctx = makeCtx(s)
    const result = applyLandContractTransferGoal(ctx, {
      holdingId: s.provinces[provinceId]?.holdingIds[0] as HoldingId,
