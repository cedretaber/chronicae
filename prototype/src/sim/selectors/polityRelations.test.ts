...
function makeCountry(overrides: Partial<Country> & { id: CountryId }): Country {
  return {
    name: 'C',
    houseIds: [],
    treasury: 0,
...
describe('polityRelations - basic Province/Polity', () => {
  it('getProvincePolity returns the country for the province', () => {
    const cid = createCountryId('c', 1)
    const pid = createProvinceId('pr', 1)
    const state = emptyState()
    state.countries[cid] = makeCountry({ id: cid })
    state.provinces[pid] = makeProvince({ id: pid, countryId: cid })
...
describe('polityRelations - getPolityProvinceIds', () => {
  it('returns provinces in the polity sorted ascending', () => {
    const cid = createCountryId('c', 1)
    const otherCid = createCountryId('c', 2)
    const state = emptyState()
    state.countries[cid] = makeCountry({ id: cid })
    state.countries[otherCid] = makeCountry({ id: otherCid })
    const pids = [createProvinceId('pr', 3), createProvinceId('pr', 1), createProvinceId('pr', 2)]
    state.provinces[pids[0]!] = makeProvince({ id: pids[0]!, countryId: cid })
    state.provinces[pids[1]!] = makeProvince({ id: pids[1]!, countryId: cid })
    state.provinces[pids[2]!] = makeProvince({ id: pids[2]!, countryId: otherCid })
...
describe('polityRelations - House → Polity', () => {
  it('getHousePolityIds returns active polities the house owns provinces in', () => {
    const c1 = createCountryId('c', 1)
    const c2 = createCountryId('c', 2)
    const inactiveC = createCountryId('c', 3)
    const hid = createHouseId('h', 1)
    const p1 = createProvinceId('pr', 1)
    const p2 = createProvinceId('pr', 2)
    const p3 = createProvinceId('pr', 3)
    const state = emptyState()
    state.countries[c1] = makeCountry({ id: c1 })
    state.countries[c2] = makeCountry({ id: c2 })
    state.countries[inactiveC] = makeCountry({ id: inactiveC, active: false })
    state.houses[hid] = makeHouse({ id: hid, provinceIds: [p1, p2, p3], seatProvinceId: p1 })
    state.provinces[p1] = makeProvince({ id: p1, countryId: c1, ownerHouseId: hid })
    state.provinces[p2] = makeProvince({ id: p2, countryId: c2, ownerHouseId: hid })
    state.provinces[p3] = makeProvince({ id: p3, countryId: inactiveC, ownerHouseId: hid })
  ...
  it('getHousePolityIds returns empty for inactive house', () => {
    ...
    state.provinces[pid] = makeProvince({ id: pid, countryId: cid, ownerHouseId: hid })
  ...
  it('getHouseProvinceIdsByPolity filters house provinces by polity', () => {
    ...
    state.houses[hid] = makeHouse({ id: hid, provinceIds: [p1, p2, p3], seatProvinceId: p1 })
    state.provinces[p1] = makeProvince({ id: p1, countryId: c1, ownerHouseId: hid })
    state.provinces[p2] = makeProvince({ id: p2, countryId: c1, ownerHouseId: hid })
    state.provinces[p3] = makeProvince({ id: p3, countryId: c2, ownerHouseId: hid })
...
describe('polityRelations - getHousePrimaryPolityId', () => {
  it('returns the polity of seatProvinceId when house owns province there', () => {
    ...
    state.provinces[seat] = makeProvince({ id: seat, countryId: c1, ownerHouseId: hid })
    state.provinces[other] = makeProvince({ id: other, countryId: c2, ownerHouseId: hid })
  ...
  it('breaks ties by development sum, then by polity id ascending', () => {
    ...
    state.provinces[p1] = makeProvince({ id: p1, countryId: c1, ownerHouseId: hid, development: 1 })
    state.provinces[p2] = makeProvince({ id: p2, countryId: c2, ownerHouseId: hid, development: 5 })
...
describe('polityRelations - getHouseSeatProvinceInPolity', () => {
  it('returns the actual seatProvinceId when it lies in the target polity and is owned by the house', () => {
    ...
    state.provinces[seat] = makeProvince({ id: seat, countryId: cid, ownerHouseId: hid })
  ...
  it('picks the province with highest development when seat is not in the target polity', () => {
    ...
    state.provinces[seat] = makeProvince({ id: seat, countryId: c1, ownerHouseId: hid })
    state.provinces[p2] = makeProvince({ id: p2, countryId: c2, ownerHouseId: hid, development: 3 })
    state.provinces[p3] = makeProvince({ id: p3, countryId: c2, ownerHouseId: hid, development: 7 })
