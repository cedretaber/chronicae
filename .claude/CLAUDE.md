...
Agent / Workflow で subagent を起動する際は、**モデルを常に明示指定する**。省略するとセッションのモデル（通常 opus）を継承し、コストが無駄になる。

### 選択基準

| タスクの性質 | 推奨モデル | 例 |
|---|---|---|
| **機械的実装** (単一関数・明確 spec・1-2 ファイル) | haiku | ボイラープレート生成、定型パターン適用 |
| **検索・探索** (Explore agent) | sonnet | シンボル検索、ファイルパターン探索 |
| **統合・判断** (複数ファイル連携・デバッグ) | sonnet | 影響範囲の調査、中規模実装 |
| **設計・アーキテクチャ** | opus | 設計判断、複雑な仕様理解 |
| **レビュー** | diff の複雑さに応じて選択 | 小さな機械的 diff → sonnet、微妙な並行性変更 → opus |
| **最終ブランチレビュー** | opus | 全体整合性の検証 |

### 判断の補足ルール

- **ターン数 > トークン単価**: 安いモデルが 2-3 倍のターン数をかけると結果的に高くつく。レビューや散文ベースの実装は sonnet を下限にする
- **plan にコード全文がある（＝写経）なら haiku**、設計判断が必要なら opus
- **大量並列 fan-out** (10+ agent) はコストが効くため sonnet 以下を優先

```typescript
// 例
Agent({ model: "haiku", ... })   // 機械的・定型作業
Agent({ model: "sonnet", subagent_type: "Explore", ... })  // 検索
Agent({ model: "sonnet", ... })  // 統合・中規模実装
Agent({ ... })                   // opus 継承 = 設計・判断が必要な作業
```
