...
- **生産（v0.55）**: RealEstateAsset は `recipeSlots`（20 slot = 100%、slot は労働配分比率で生産量乗数ではない）で 23 種 recipe（§3.2c）を採用する。holding の employed POP を **各 PopStratum** ごとに per-asset の capacity×stratum weight 比で按分（`computeAllocatedLaborByAsset`、RealEstateAsset は複数 stratum を同時雇用、§13.4。upper は雇用しない）。`recipeLabor = allocatedLabor × (slot/totalSlots)`、`potentialOutput = recipeLabor × baseOutputPerLabor × recipeScaleMultiplier × productionFacilityModifier × polityControlModifier × laborTypeFulfillmentModifier`。**asset.level は雇用枠（`capacityPerLevel × level` = 施設サイズ）にのみ効き、労働あたり生産性には掛けない**。development modifier は production path から撤去。`laborTypeFulfillmentModifier`（質的充足、§6.3c.2）は recipe の理想 PopType 構成と実構成のズレを floor 0.70 付きで効率に反映。`inputEfficiency`（admin role coverage）は **output 倍率ではなく input requirement の軽減**として別経路（下限 0.5）。
- **地形と生産（v0.55 の効き方）**: terrain / feature / `landQuality` は **asset の雇用容量（capacity）の乗数**としてのみ生産に効く（`realEstateTerrainCapacityMultiplier` / `realEstateFeatureCapacityMultiplier` / `landQuality` を assetTerm に乗算、§4.1 / §9）。容量が増えれば雇用できる POP が増え総産出が増えるが、**1 労働あたり産出（`baseOutputPerLabor`）は地形非依存**。なお RealEstateKind 単位の `allowedTerrains`（§3.2c）は asset の建設可否ゲートで、これも産出量には効かない。fishing_hut は farm recipe の一つで水辺 feature を要求しないため、内陸 farm でも採用でき魚を産する。**未実装の将来枠（型のみ・enforce しない）**: `ProductionRecipe.terrainOutputModifier`（地形 → per-labor 産出倍率）/ recipe 単位 `allowedTerrains` / `allowedFeatures`。これらは「水辺の漁場は高効率」「平原の穀物は高効率」のような地形ボーナスを per-labor 生産性として表現するための器だが v0.55/56 では未配線（§14.1 将来課題）。
...
- **consumerCost の二層性（実装上の最重要分岐）**: recipe の **input buyOrders は実コスト**で asset の `inputCost` → `netRevenue` に効く。一方 POP の **NeedCategory buyOrders は観察値のみ**（PopGroup.cash 不在のため cash から引かない。POP welfare は shortage / high-price penalty 経由でのみ動く）。この二層を取り違えると POP cost の二重計上 or recipe inputCost の脱落を招く。
...
- **laborTypeFulfillmentModifier（質的充足、§14.3）**: recipe は理想 PopType 構成 `RecipeLaborDemand`（`RECIPE_LABOR_PROFILES`）を持つ（hard gate ではない soft modifier）。asset の実 PopType 構成（stratum weight × holding 内 PopType 比率で近似）と理想のズレを `floor + (1−floor) × weightedCoverage` で効率に反映（`laborTypeFulfillmentFloor=0.70`）。`weightedCoverage = Σ_t min(actualShare_t, idealShare_t)`（理想/実分布のヒストグラム交差、∈[0,1]）。`inputEfficiencyMultiplier = 1 − Σ_admin(inputEfficiencyBonus_t × coverage_t)`（下限 0.5）で input requirement を軽減（output には掛けない）。`maxRatioTo` / `throughputBonus` は将来用 metadata で v0.55 では未使用。
