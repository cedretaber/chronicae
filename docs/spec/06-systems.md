...
- **balance-defer（worldgen の職能 seed 不一致）**: worldgen は stratum 容量を `WORLDGEN_POP_TYPE_DISTRIBUTION`（全 holding 共通: peasants 55% / artisans 15% 等）で分割して POP を seed するため、都市 holding でも小作農が多く seed され、工房の職人枠が初期は埋まりにくい（mobility が peasants→…→artisans へ変換するまで都市生産が立ち上がりにくい）。soft floor 撤去と相まって「職人ゼロの工房は産出ゼロ」になり得る。施設構成に応じた worldgen seed（都市は職人主体）はバランスフェーズで検討する。現状 150年×4seed は完走し integrity green。
