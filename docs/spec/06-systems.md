...
- **aim 生成自体もボトルネックをトリガにする（到達性）**: `buildProjectFieldsForAim` の分岐は `develop_owned_holding` Aim が作られて初めて走る。その Aim 生成（goalSelectors §6.36 相当の `develop_owned_holding`）は従来 `devDeficit || hasCapacityPressure || hasEmploymentSlack` のみが条件で、食料束縛 holding（人口 < capacity で**全員就業＝失業者 0 → slack 偽**・低稼働で capacityPressure 偽・dev≥40 で devDeficit 偽）はこの3条件が全偽になり Aim が生成されず、ボトルネック分岐に到達できないデッドゾーンがあった。そこで `selectProductivityImprovementForBottleneck(...) !== undefined` を **aim 生成条件の最後の disjunct** に追加（`||` で短絡し、他トリガが立つ通常 holding では評価しないので perf 影響なし）。これで `hasEmploymentSlack` 非依存が aim 生成・project 種別決定の両層で成立する。自己調整的（改良 → 食料 output↑ → CC↑ → 人口↑ → pressure 低下 → capacity 建設へ戻る）で、`canBuildHoldingImprovement` の `currentLevel < maxLevel`（灌漑 max 3）が自然な上限。RNG 不使用＝決定性維持。
...
3. 付け替え後の owner を再 read し `!isOrganizationActive(owner)` → **disband**（owner 消滅）。
...
- **減税系 aim の受諾見込みゲート**: `improve_owned_contract_terms` / `eliminate_overlord_contract`（いずれも vassal → grantor への減税要求）は、対象契約の grantor（宗主）が polity でありかつ `predictPressureResponseStance(self, grantor) === 'resist'`（grantor が自分の `PRESSURE_RESIST_POWER_RATIO`=1.2 倍以上強い）の場合、候補に入れない。feudal chain 上、宗主はほぼ常に臣下より強く resist 確実なので、これを欠くと弱い臣下が「勝ち目のない減税要求」を量産し、外交劇は起こすが全て status_quo に終わる（「外交劇は起こすが何も変わらない」連発）。`predictPressureResponseStance`（`selectors/pressureStanceSelectors.ts`）は `choose_stance` の実 stance 決定（§6.57 / 後述）と play 開始ゲート（§6.42）で共有する単一の式で、将来 `getOrganizationMilitaryPower` の算出が変われば予測と実応答の両方へ自動反映される。
...
**市場 injection（§14）**: `tradePlanningSystem`（resourceEconomy 直前・月次）が各 active route の計画量を算出。計算は純関数 `computeExpectedRouteEconomics`（`merchantSelectors.ts`）に集約し、TradePlanning / Accounting / 候補評価 / probe で drift しない。**v0.61 交易路修正**: 旧式の raw `targetImportDemand`（order 差分の絶対量＝p50=0.12 で大半の route が搬送量ゼロ）を廃止し、前月 **`smoothedPrice`**（EWMA weight=0.75）の価格差ベースに転換。`spreadRatio = spread / averagePrice`・`spreadFactor = clamp(spreadRatio / fullUtilizationSpreadRatio, 0, 1)`・`plannedQuantity = expectedUnitMargin ≤ 0 ? 0 : min(throughput, sourceExportable) × spreadFactor`。route は `plannedBuyPrice`/`plannedSellPrice`/`plannedExpectedUnitMargin` を保存し Accounting に渡す。resourceEconomy で **非対称注入**: source 側 export 需要を清算前 demand へ、target 側 import 供給を `computeResourcePrice` 直前のローカル sell へ加算。これで route が price を均す方向に作用する。

**会計（§15/§16）**: `merchantCompanyAccountingSystem`（月次）が route profit + commerce revenue を gross に集計。route profit は **planning price 基準**（TradePlanning が保存した `plannedBuyPrice`/`plannedSellPrice` で arbitrage/serviceFee を計算・自己注入利益消失を回避）。maintenance は **固定+従量**（`tradeRouteFixedMaintenanceCostByLevel` + `quantity × tradeRouteVariableMaintenanceCostPerUnit`）。commerce revenue は前月 snapshot・施設 kind 別 cap。§16.6 で **wage（雇用 POP へ mint）/ upper 配当（雇用 patricians へ mint）/ owner 配当（House.wealth）/ retained（treasury）** へ分配。**carve==mint**（mint 先不在 pool は treasury 残置で money 保存）。
...
**自律化（§17/§18）**: `merchantCompanyDecisionSystem`（年次）が treasury / smoothedProfit / 市場シグナルで route 開設・拡張・閉鎖（**close/replace は即時**）・支店建設・本店増築を決める。**v0.61 交易路修正**: route 開設の候補評価を `computeExpectedRouteEconomics(level=1)` の `expectedMonthlyProfit` ベースに転換（旧 order 積スコアを撤去）。upgrade 判定は `smoothedProfit > 0` かつ高 utilization かつ `level+1` の利益向上が閾値以上。**HQ 成長前倒し**（決定木 b2）: treasury > budget かつ黒字・高 utilization・route.level==hq.level の route が存在するとき、支店建設より先に HQ 増築を行う。slotCap に進行中 `open_trade_route` Project 数を加算し二重 open を防止。close 閾値は `merchantRouteCloseSmoothedProfitThreshold` = -0.1（固定維持費 0.15 に追従・idle route の smoothedProfit 収束先 ≈ -fixedMaintenance を確実に下回る値）。**lean 逸脱①**: 設計ドラフトは商会を Goal/Aim の owner として統合する想定だったが、実装は crisis と同じ **system-origin の born-complete Project**（status=completed で生成→次 tick の projectOutcomeSystem が 4 ProjectKind outcome を発火→route/branch 生成→purge）を採り、Goal/Aim entity を介さない。cost は生成時に treasury から drain（投資額）。multi-week 実行・funding 集金（`projectFundingSelectors` の merchant_company 分岐）は未配線（future）。

**lifecycle（§20）**: `cleanupMerchantSystem`（weekly）が破産（accounting の `distressSince` で treasury<閾値 & smoothedProfit<0 の連続週を追跡）→ dormant 化（`MerchantCompanyStatus` の `'bankrupt'` は v0.61 現在未使用。将来 bankrupt と dormant を区別する場合の予約枠） + terminal entity（closed route/branch・dormant/dissolved 商会＋share）の retention purge。`merchantCompanyFoundingSystem`（年次）が active 商会ゼロの state へ cooldown + production gate で再興（**ctx.rng で person 生成**・ID は `ctx.nextPersonIndex/nextHouseIndex` の live カウンタから採る＝state.nextPersonIndex は worldgen 専用で stale）。house 断絶時は `worldStructureExtinction` が owner 商会を dissolved 化（§20.4・同 tick）。
