...
- **config**: `wageShareOfNetRevenue` / `wageRoleWeightByRole` / `needSatisfactionTierWeight` / `needSatisfactionSmoothing` / `worldgenPopMoney*`。値は暫定（balance フェーズで調整）。
- **将来**: 労働需給に応じた動的賃金率、施設別の富格差可視化、完全保存市場（§13 将来課題）。
