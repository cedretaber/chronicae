...
- **施設(improvement)労働者の俸給 supplement（holding 収入から）**: 施設(manor_house/town_hall/storage)は収益を生まないので、施設に雇用される POP の給料は holding 収入(owner 取り分)から確保する。生産者の賃金を希釈しないよう給与水準は「生産 asset の労働者のみで算出した per-capita」に合わせる。実装は **stratum 単位**: holding の `facCap_s`(施設雇用枠容量) / `prodCap_s`(生産 asset 雇用枠容量) を level 込みで求め、各 POP の生産賃金 `prodWage` に `facCap_s/prodCap_s` を掛けた額を施設俸給として mint する(= 施設分の頭数に同 per-capita で払うのと等価、slot 充足率に依らず成立)。`prodCap_s=0` の stratum(upper)は対象外(別途配当)。supplement は asset へ `max(0,netRevenue)` 比で按分し `wageShare` に合算して LandRevenue が控除する(施設俸給も労働コスト=owner 収入から)。背景: 施設労働者(兵士・事務官・家士等、主に lower/middle)は従来も生産 asset の賃金プールを同階層で分け合っていた(=生産者の賃金を希釈)が、これを「生産者は希釈なし＋施設労働者は holding 収入から別途」に整理した。owner 影響は実測で施設俸給 ≈ netRevenue の 17.5%(owner 取り分 50%→~29.5%)と小さくないが、施設労働者の購買力増で welfare はむしろ改善(needSat 3/4 seed 上昇)。
